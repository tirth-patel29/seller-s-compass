        ...
        <div className="grid gap-6 md:grid-cols-3">
          <div className="md:col-span-2 space-y-6">
            <div className="rounded-xl border border-border bg-card p-6 shadow-sm">
              ...
              {!exportOrder ? (
              ...
              ) : exportOrder.status === 'submitted' ? (
                <div className="rounded-lg border border-border bg-card p-4 shadow-sm">
                  <div className="flex justify-between items-start mb-4">
                    <div>
                      <h3 className="font-semibold text-slate-800">DNK Export</h3>
                      <p className="text-sm text-muted-foreground mt-1">Submitted to {exportOrder.dnk}</p>
                    </div>
                    <span className="bg-blue-700 text-white text-xs font-bold px-2 py-1 rounded">SUBMITTED</span>
                  </div>
                  <div className="space-y-2 mb-4 text-sm text-slate-700">
                    <p className="flex items-center gap-2"><CheckCircle2 className="size-4 text-success" /> Documents generated</p>
                    <p className="flex items-center gap-2"><CheckCircle2 className="size-4 text-success" /> DNK requested</p>
                    <p className="flex items-center gap-2"><CheckCircle2 className="size-4 text-success" /> Shipment created</p>
                  </div>
                  <div className="flex gap-3 mt-5">
                    <Button variant="outline" asChild size="sm" className="flex-1">
                      <Link to="/seller/orders/$id/export/documents" params={{ id: order.id }}>View Documents</Link>
                    </Button>
                  </div>
                </div>
                ...
                <div className="rounded-lg border border-border p-5">
                  <div className="flex justify-between items-start mb-2">
                    ...
                    {(() => {
                      const checklistKeys = ["seller", "product", "packageInfo", "destination", "exportInfo"] as const;
                      const total = checklistKeys.length;
                      const completed = checklistKeys.filter(k => exportOrder.checklist[k]).length;
