...
import { getOrderById, getExportOrderByOrderId, updateExportDetails, generateExportDocuments, getProductById } from '@/services/mockServices'
              ...
              <div className="flex items-center justify-between bg-success/10 p-4 rounded-lg border border-success/20">
                ...
                <Button 
                  onClick={async () => {
                    ...
                    try {
                      if (order && exportOrder && product) {
                        const doc = documentService.generateExportPackagePdf(order, exportOrder, product);
                        doc.save(`DNK-${order.id}-Export-Package.pdf`);
                      } else {
                        throw new Error("Missing data to generate PDF");
                      }
                      await generateExportDocuments(id);
                      navigate({ to: "/seller/orders/$id/export/documents", params: { id } });
                    } catch (e) {
                      console.error("PDF generation failed:", e);
                      toast.error("Unable to generate PDF. We couldn't generate the export document package. Try Again");
                    } finally {
                ...
                >
                  {generatingDocs ? "Generating PDF..." : "Generate Export Documents"}
                </Button>
              </div>
            ) : (
